...
export const recipientActions = {
	createMandant: async ({ request }: RequestEvent) => {
	...
	updateMandant: async ({ request }: RequestEvent) => {
		const formData = await request.formData();
	...
	deleteMandant: async ({ request }: RequestEvent) => {
		const formData = await request.formData();
		const id = formData.get('id');
		if (typeof id !== 'string' || !id) return fail(400, { error: 'Ungültige ID.' });
	...
	createEinrichtung: async ({ request }: RequestEvent) => {
		const formData = await request.formData();
		const name = formData.get('name');
	...
	updateEinrichtung: async ({ request }: RequestEvent) => {
		const formData = await request.formData();
		const id = formData.get('id');
		const name = formData.get('name');
	...
	deleteEinrichtung: async ({ request }: RequestEvent) => {
		const formData = await request.formData();
		const id = formData.get('id');
		if (typeof id !== 'string' || !id) return fail(400, { error: 'Ungültige ID.' });
	...
	createFachabteilung: async ({ request }: RequestEvent) => {
		const formData = await request.formData();
		const name = formData.get('name');
	...
	updateFachabteilung: async ({ request }: RequestEvent) => {
		const formData = await request.formData();
		const id = formData.get('id');
		const name = formData.get('name');
		const email = formData.get('email');
	...
	deleteFachabteilung: async ({ request }: RequestEvent) => {
		const formData = await request.formData();
		const id = formData.get('id');
		if (typeof id !== 'string' || !id) return fail(400, { error: 'Ungültige ID.' });
